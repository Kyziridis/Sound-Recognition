...
Run it with python name.py "/data/s2077981/Birds" "/data/s2077981/birds"
"""
import sys
import random
import numpy as np
...
print("\nThere are 4 datasets to import contatining: 56, 150, 500, 8000 wavs  >_")
print("\nChoose one datset to import for the RNN                              >_")
...
tr_ind = random.sample(all_ind , 5600) 
#
...
def RNN(x, weight, bias):
    ...
    for iiLyr in range(2):
        stacked_rnn.append(tf.contrib.rnn.LSTMCell(num_units=n_hidden, state_is_tuple=True))
    MultiLyr_cell = tf.contrib.rnn.MultiRNNCell(cells=stacked_rnn, state_is_tuple=True)
